...
import com.example.tracker_data.remote.api.OpenFoodApi
...
class TrackerRepositoryImplTest {
    ...
    @Before
    fun setUp() {
        // we instantiate all of our mocked objects needed for network calls
    ...
    // will destroy mocked server after every test
    @After
    fun tearDown() {
    ...
    // runBlocking is totally perfect to be used in tests
    @Test
    fun `Search food, valid response, returns results`() = runBlocking {
        // we enqueue our response, the code we expect and the body we expect to be returned
        ...
        // we expect our server to be success call
